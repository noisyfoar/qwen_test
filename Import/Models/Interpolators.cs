...
namespace NPFGEO.ShellExtension.Formats.LIS.Dialogs.Import.Models
{
    ...
    public class NonInterpolator : IInterpolator
    {
        public void Interpolate(Curve orig, IList<double> x_new, IList<double?> y_new)
        {
            ...
            for (int i = 0; i < x_new_count; i++)
            {
                while (true)
                {
                    if (index == x_orig_count_minus_one || orig.DepthMatrix[0, index] >= x_new[i]) break;
                    index++;
                }
    ...
    public class LinearInterpolator : IInterpolator
    {
        ...
        public void Interpolate(Curve orig, IList<double> x_new, IList<double?> y_new)
        {
            if (orig == null || x_new == null || y_new == null) throw new NullReferenceException();

            var orig_count = orig.DepthMatrix.Rows;
            var new_count = x_new.Count;

            if (new_count != y_new.Count) throw new ArgumentException();
            ...
            {
                x_new_cur = x_new[i];
                if (index > orig_count_minus_one) break;

                while (true)
                {
                    if (index == orig_count_minus_one || x_orig_next >= x_new_cur) break;
    ...
    public class NextNeighborInterpolator : IInterpolator
    {
        ...
        public void Interpolate(Curve orig, IList<double> x_new, IList<double?> y_new)
        {
            ...
            for (int i = 0; i < x_new_count; i++)
            {
                while (true)
                {
                    if (index == x_orig_count_minus_one || orig.DepthMatrix[0, index + 1] >= x_new[i]) break;
                    index++;
                }
                ...
                else if (index == x_orig_count_minus_one)
                {
                    ...
                    if ((x_new[i] - orig.DepthMatrix[0, index]) < (orig.DepthMatrix[0, index + 1] - x_new[i]))
                        y_new[i] = System.Convert.ToDouble(orig.DataMatrix[0, index]);
                    else
